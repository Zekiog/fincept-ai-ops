...
import uuid
from apps.fincept_aiops.validation import validate_research_note, is_valid_confidence, has_minimum_sources
...
class ResearchPipeline:
    ...
    def run(self, research_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        order_intent = {"asset": signal["asset"], "side": signal["side"],
                        "thesis": signal["thesis"], "size_pct": signal.get("size_pct", 0.05)}
        risk_result = self.risk.evaluate(order_intent, {})
        briefing = {
            "date": datetime.utcnow().strftime("%Y-%m-%d"),
            "research_summary": research_input.get("summary"),
            "signal": signal, "risk": risk_result,
        ...
        self.audit.append({"actor": "research_pipeline", "action": "pipeline_complete",
                           "asset": signal["asset"], "side": signal["side"],
                           "confidence": research_input.get("confidence"), "risk_status": risk_result["status"]})
        return {"ok": True, "research_note": research_input, "signal_candidate": signal,
                "risk_decision": risk_result, "briefing_packet": briefing}
