...
class StrategyLab:
    """Generates signal candidates from validated research notes."""
    ...
    def generate_signal(self, research_note: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(bull) >= len(bear):
            side = "buy"
            thesis = bull[0] if bull else "bullish technical signal"
        else:
            side = "sell"
            thesis = bear[0] if bear else "bearish technical signal"
